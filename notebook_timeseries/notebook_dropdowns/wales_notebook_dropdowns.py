...
def draw_site_from_map(gpd_df_sub):
    ...
    html = HTML()
    html.value = "<b> Draw an area on the map below, using the left-hand toolbar, within the highlighted boundry.</b>"
    ...
    # Overlay: Street/city labels
    labels_layer = TileLayer(
        url="https://server.arcgisonline.com/ArcGIS/rest/services/Reference/World_Transportation/MapServer/tile/{z}/{y}/{x}",
        name="Street/City Labels",
        attribution="Tiles © Esri",
        opacity=0.8  # adjust transparency so imagery is visible
    )
    ...
    attribution="Tiles © Esri",
    opacity=1.0  # Set opacity to 1.0 (100%)
    )
